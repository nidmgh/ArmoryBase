...
### 4. 计算和储存分离
上面提到的upper和lower提供了比较好的模块分解。lower可以升级变化的不多，除了枪托外，最需要升级的就是板机。而upper的范围可是多了。下面是网上搜的一些：
![AR15 upper-gallery](../images/05_AR-15-upper-gallery.jpeg)
明显的在枪管长度上，从7寸9寸的短突，到14.5寸的M4/M16标准军用突击步枪(assualt rifle)，到18寸20寸的中距离狙击枪。并且可以根据业务用途，安装不同的附件。比如M16一般用铁锚或红点，几十到200米距离够用了，长的加好的scope，4～500米上靶子。

#### 4.1 AR-15 A and B, and field test
这两把看起来区别比较大，但产品架构完全一致的。它们的upper和lower可以轻松互换：不用任何辅助工具，10秒内完成。
![AR15 A and B](../images/05_RifleAandB.jpg)

* Rifle A: 1. 可调整枪托; 2. 18寸一般的枪管; 3.  机械瞄准器：Iron sight 铁锚； 4. TCO: ~$600
* Rifle B: 1. 固定枪托; 2. 20英寸精准枪管; 3.  光学瞄准器：4-14倍 scope; 4. TCO:  ~$1600； 5. 容易配搭各种附加，比如图中的前支架bipod

从各个维度， Rifle B肯定是高档货。那它是不是所有业务的首选呢？

#### 4.2 测试结果

表面结果也如是： 
![AR15 A and B](../images/05_ResultAandB.jpg)

在同一个时间同样子弹ammo的情况下，坐姿测试5发子弹的precision精确, 而不是accuracy准确。
具体这两个技术指标的区别可以参考这篇[夏威夷大学网上](https://manoa.hawaii.edu/exploringourfluidearth/physical/world-ocean/map-distortion/practices-science-precision-vs-accuracy)的解释。
上半部：Rifle B在100码（约90米）采用支架支撑的情况下，六组五发的结果，均小于1.1英寸，最好的一组是0.589" 。如果前部采用沙袋支撑，我一般可以做到0.8英寸(2厘米)以内, 照片中可以看到硬币quarter(直径0.955英寸)为参照物。
下半部：Rifle A在25码肘部支撑情况下，七组的结果（左下是四发，结果出奇的好，我都不敢相信），贴的目标是3英寸直径的。本来是计划与上面同样作100yard的标准测试，但是举起后发现在100yard根本看不到十字和3寸标。所以不得已认怂，挪到25码。结果是25码，1～1.6寸之间。
如果做个**表面的**简单的几何计算的话，Rifle B的结果比Rifel A好**6X**。

#### 4.3 选型（always a but)

Rifle B除了成本高以外，至少有三大缺点：
* 重量和体积： 重50%， 大约重2公斤；长30%
* 视野：scope越高倍数，使用者的视野越狭小；而铁锚要好很多
* 维护：越精准的仪器对于保养和原料要求越苛刻，否则轻者无法发挥其特性(精确和准确降低），重者直接当机（卡壳爆膛）

所以从适用场景的角度，Rifle B更适合比赛（其实也不是了，比赛用的是6千刀入门的定制品）；Rifle B更适合打猎和野外生存（当然也不是，打猎的和survival rifle又有各自的技术要求）

每每想到这里，我总是会回想起与客户的交流。数据库的客户决策者，一般都是有技术背景的，虽然不一定是数据库专家。其实这类客户是很容易打交道，交朋友的。获取客户认可和信任，只要老老实实的了解客户的业务情况，清清楚楚的把不同数据库的能力和优缺点摆在明面，就可以了。

### 5. 尾巴
AR-15平台与PG有同样的扩展性： AR平台是针对556子弹设计的。借助其模块化设计，现在覆盖多种流行的弹药，比如308的AR-10，9mm的AR-9, 也可以适用7.62X39mm和5.45X39mm， AR-15可以装个转换器支持22lr。

有趣的是，同样作为业界极知名的产品，它们在欢迎度（装机量和生产量）又都是老二，分别落后于AK平台和MySQL。有些同病相怜的感觉

长周末过完了，写到这里就是个结尾。只是觉得很多工程设计，架构原理在底层是相通的。把脑海里的的头绪，有时间落在纸上，便是幸事。其实我了解只是皮毛，有兴趣的同学网上的资料是非常多的。
